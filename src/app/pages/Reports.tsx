...
export const Reports: React.FC = () => {
  const { sales, products, purchases, refunds } = useAppContext();
  ...
  const stats = useMemo(() => {
    ...
    const periodRefunds = refunds.filter(r => new Date(r.date) >= startDate);
    const totalRefunds = periodRefunds.reduce((sum, r) => sum + (r.total || 0), 0);
    ...
    const cashFlow = netRevenue - totalPurchaseCost;
  ...
  }, [sales, products, purchases, period, refunds]);
  ...
  const salesByCategory = useMemo(() => {
    ...
    // build refunds by product to subtract refunded revenue
    ...
    sales.forEach(sale => {
      sale.items.forEach(item => {
        const product = products.find(p => p.id === item.productId);
        if (product) {
          const refunded = refundsByProduct[item.productId] || 0;
          const current = (categoryData[product.category] || 0) + item.subtotal;
          categoryData[product.category] = current - refunded;
        }
      });
    });
  ...
  const dailySalesTrend = useMemo(() => {
    ...
    for (let i = 13; i >= 0; i--) {
      ...
      days.push({
        ...
        ventas: Math.round(revenue - refundTotal),
        ganancias: Math.round((revenue - refundTotal) - cost)
      });
    }
    return days;
  }, [sales, products, refunds]);
  ...
  const topProducts = useMemo(() => {
    ...
    Object.keys(productStats).forEach(pid => {
      productStats[pid].revenue = Math.max(0, productStats[pid].revenue - (refundsByProduct[pid] || 0));
    });
