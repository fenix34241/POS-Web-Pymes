...
export const Dashboard: React.FC = () => {
  const { sales, products, refunds } = useAppContext();
  ...
  const stats = useMemo(() => {
    ...
    const todayTotalRaw = todaySales.reduce((sum, s) => sum + s.total, 0);
    const weekTotalRaw = weekSales.reduce((sum, s) => sum + s.total, 0);
    const monthTotalRaw = monthSales.reduce((sum, s) => sum + s.total, 0);

    // refunds in periods
    const todayRefunds = refunds.filter(r => new Date(r.date).toDateString() === todayStr);
    const weekRefunds = refunds.filter(r => new Date(r.date) >= weekAgo);
    const monthRefunds = refunds.filter(r => new Date(r.date) >= monthAgo);

    const sumRefunds = (arr: any[]) => arr.reduce((s, r) => s + (r.total || 0), 0);

    const todayRefundTotal = sumRefunds(todayRefunds);
    const weekRefundTotal = sumRefunds(weekRefunds);
    const monthRefundTotal = sumRefunds(monthRefunds);

    const todayTotal = todayTotalRaw - todayRefundTotal;
    const weekTotal = weekTotalRaw - weekRefundTotal;
    const monthTotal = monthTotalRaw - monthRefundTotal;
    ...
    const todayProfitRaw = calculateProfit(todaySales);
    const monthProfitRaw = calculateProfit(monthSales);

    // adjust profit by refunds: for each refunded item, profit change = (product.cost * qty) - refundAmount
    const refundsProfitAdjustment = (refundList: any[]) => {
      let adj = 0;
      refundList.forEach(ref => {
        if (ref.items && Array.isArray(ref.items)) {
          ref.items.forEach((it: any) => {
            const product = products.find(p => p.id === it.productId);
            const cost = product ? product.cost * (it.quantity || 0) : 0;
            const refundAmount = it.refundAmount || 0;
            adj += cost - refundAmount;
          });
        }
      });
      return adj;
    ...
    const todayProfit = todayProfitRaw + refundsProfitAdjustment(todayRefunds);
    const monthProfit = monthProfitRaw + refundsProfitAdjustment(monthRefunds);
  ...
  const salesByDay = useMemo(() => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayRefunds = refunds.filter(r => new Date(r.date).toDateString() === dateStr);
      const refundTotal = dayRefunds.reduce((s, r) => s + (r.total || 0), 0);
      
      data.push({
        day: days[date.getDay()],
        ventas: Math.round(total - refundTotal)
      });
    }
  ...
  const topProducts = useMemo(() => {
    ...
    // subtract refunds per product
    const refundsByProduct: { [key: string]: number } = {};
    refunds.forEach(r => {
      if (r.items && Array.isArray(r.items)) {
        r.items.forEach((it: any) => {
          refundsByProduct[it.productId] = (refundsByProduct[it.productId] || 0) + (it.refundAmount || 0);
        });
      }
    });

    Object.keys(productSales).forEach(pid => {
      productSales[pid].revenue = Math.max(0, productSales[pid].revenue - (refundsByProduct[pid] || 0));
    });
