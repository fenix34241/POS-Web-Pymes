...
import { Product, Sale, Purchase, Supplier, InventoryMovement, User, Refund } from '../types';
import {
  authApi,
  userApi,
  productApi,
...
interface AppContextType {
  ...
  addRefund: (refund: {
    saleId: string;
    items: Array<{ saleItemId: number; quantity: number; refundAmount: number }>;
    reason: string;
    reasonDetail?: string;
  }) => Promise<void>;
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const refreshData = useCallback(async () => {
    try {
      const [prodData, salesData, purchData, suppData, movData, usersData, refundData] = await Promise.all([
        productApi.getAll(),
        saleApi.getAll(),
        purchaseApi.getAll(),
        supplierApi.getAll(),
  ...
  const addRefund = useCallback(async (refundData: {
    ...
    const created = await refundApi.create(refundData);
    setRefunds(prev => [created, ...prev]);
  }, []);

  const approveRefund = useCallback(async (id: string) => {
    await refundApi.approve(id, currentUser?.id || '');
    // Refresh refunds list
    const updatedRefunds = await refundApi.getAll();
  ...
  const rejectRefund = useCallback(async (id: string) => {
    await refundApi.reject(id);
    // Refresh refunds list
    const updatedRefunds = await refundApi.getAll();
    setRefunds(updatedRefunds);
  }, []);
